 ...
 *                         Offsets and lengths are 4-byte values (vs 8 bytes in the vl.c benchmrak).
...
int create_dsets(hid_t file, hid_t file_comp, hid_t file_struct, hid_t file_struct_comp)
{
    ...
    p = the_pairs = (uint *)malloc(2 * hand.nelemts * sizeof(uint));
    ...
    dset = H5Dcreate2(file_struct, OFFSET_LENGTH_DSET_NAME, H5T_NATIVE_INT, dataspace, H5P_DEFAULT, dcpl, H5P_DEFAULT);

    /* Create a new dataset with compression */
    dset_compressed = H5Dcreate2(file_struct_comp, OFFSET_LENGTH_DSET_COMP_NAME, H5T_NATIVE_INT, dataspace, H5P_DEFAULT, dcpl_compressed, H5P_DEFAULT);

    /* Write the data to the dataset */
    H5Dwrite(dset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, the_pairs);
