 ...
 * mm2h5-r-struct.c
 *
 * This program reads triplets of tow unsigned long long integers and flowting-point number from a text file
 * and writes them into a two-dimensional chunked and compressed HDF5 dataset.
 *
 * Usage:
 *     ./mm2h5-r-struct <file.txt> <XC> <YC> <GZIP_LEVEL> [-v]
 *
 * Behavior:
 ...
 *   - The output HDF5 file is named <file_gzipLEVEL_struct.h5>.
 ...
 *
 * Example:
 *     ./mm2h5-r-struct data.txt 10 10 6 -v
 *     → creates data_gzip6_struct.h5, writes with GZIP=6, prints progress reports
 *
 * Dependencies:
                       ...
                       unsigned long long *y_value)
{
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (*ptr == '%' || *ptr == '\0' || *ptr == '\n')
            continue;
        if (sscanf(ptr, "%llu %llu %lf", &x, &y, &z) == 3) {
            found = 1;
            break;
        }
    }
    ...
    double             *c = malloc(capacity * sizeof(double));
    if (!a || !b || !c) {
    ...
    *y_value = y;
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        double z2;
        if (sscanf(ptr, "%llu %llu %lf", &x2, &y2, &z2) != 3)
            continue;
        if (y2 != current_y) {
        ...
        if (count == capacity) {
            ...
            c = realloc(c, capacity * sizeof(double));
            if (!a || !b || !c) {
...
int main(int argc, char *argv[])
{
    ...
    unsigned int gzip_level = atoi(argv[4]);
    size_t       cd_nelmts  = 1;
    ...
    if (dot) *dot = '\0';
    snprintf(h5_file, sizeof(h5_file), "%s_gzip%d_struct.h5", base_name, gzip_level);
    ...
    H5Pset_struct_chunk(dcpl, 2, chunk, H5D_SPARSE_CHUNK);
    if (gzip_level > 0) {
        H5Pset_filter2(dcpl, H5_SECTION_SELECTION, H5Z_FILTER_DEFLATE, H5Z_FLAG_OPTIONAL, cd_nelmts,
                           &gzip_level);
        H5Pset_filter2(dcpl, H5_SECTION_FIXED, H5Z_FILTER_DEFLATE, H5Z_FLAG_OPTIONAL, cd_nelmts,
                           &gzip_level);
    }

    hid_t dset_id = H5Dcreate2(file_id, "data", H5T_NATIVE_DOUBLE, space_id,
    ...
    unsigned long long *a = NULL, *b = NULL;
    double *c = NULL;
    unsigned long long y_value;
    size_t n;
    ...
    while ((n = read_next_group(fp, &a, &b, &c, &y_value)) > 0) {
        ...
        H5Dwrite(dset_id, H5T_NATIVE_DOUBLE, mspace, dspace, H5P_DEFAULT, c);
        H5Sclose(mspace);
        H5Sclose(dspace);
    ...
    if (total_points <= VERIFY_ALL_LIMIT) {
        ...
        for (size_t i = 0; i < total_points; i++) {
            ...
            double value = 0;
            H5Dread(dset_id, H5T_NATIVE_DOUBLE, mspace, fspace, H5P_DEFAULT, &value);
            if (fabs(value - points[i].c) > EPSILON)
                mismatches++;
        ...
        for (int i = 0; i < VERIFY_POINTS; i++) {
            ...
            double value = 0;
            H5Dread(dset_id, H5T_NATIVE_DOUBLE, mspace, fspace, H5P_DEFAULT, &value);
            printf("  (%llu,%llu): expected=%lf, read=%lf %s\n",
                   points[idx].a+1, points[idx].b+1,
                   points[idx].c, value,
                   (fabs(value - points[idx].c) < EPSILON) ? "OK" : "MISMATCH");
