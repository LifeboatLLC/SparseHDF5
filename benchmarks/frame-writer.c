 ...
 * frame-writer.c
 *
 * This program creates an HDF5 file containing a 2D chunked dataset.
 *
 ...
 *
 * Example:
 *   ./frame-writer X=100 Y=80 XC=20 YC=20 m=20 \
    ...
    for (hsize_t cx = 0; cx < n_chunks_x; cx++) {
        for (hsize_t cy = 0; cy < n_chunks_y; cy++) {
            ...
            if (random_pattern) {
                ...
                //printf(" Total elements written in a chunk %llu \n", total_elems);
                ...
                for (hsize_t i = 0; i < total_elems; i++) {
                    ...
                    //printf(" Chunk buffer element %llu is %d \n", i, chunk_buf[i]);
                } 
