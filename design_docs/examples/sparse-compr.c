/******************************************************************
  ...
  This example shows how to read and write compressed sparse data.  
...
main(void)
{
    ...
    /*
     * Create the dataset for storing sparse data. Only data section will be compressed with GZIP.
     */
    dcpl   = H5Pcreate(H5P_DATASET_CREATE);
    status = H5Pset_struct_chunk(dcpl, 2, chunk, H5D_SPARSE_DATA, NULL, NULL); 
    status = H5Pset_filter2(dcpl, H5_SECTION_FIXED, H5Z_FILTER_DEFLATE, H5Z_FLAG_OPTIONAL, 4, &val); 

    /* To compress selection section use
     * status = H5Pset_filter2(dcpl, H5_SECTION_SELECTION, H5Z_FILTER_DEFLATE, H5Z_FLAG_OPTIONAL, 4, &val); 
     *
     * To compress all sections with the same compression use
     * status = H5Pset_filter2(dcpl, H5_SECTION_NUM, H5Z_FILTER_DEFLATE, H5Z_FLAG_OPTIONAL, 4, &val); 
     */ 
